...
Stage 1 Backend Application for the Smart Betting System.
...
Stage 1 maintains a real-time WebSocket connection with the DB Manager for:
...
#### Outgoing Events (Stage 1 → DB Manager)
- `stage1-connect`: Identifies Stage 1 connection
- `request-game-data`: Requests latest game data for a stage
- `bet-placed`: Notifies DB Manager of bet placements

#### Incoming Events (DB Manager → Stage 1)
...
```
Manually requests real-time game data for the specified stage (default: 'a').
