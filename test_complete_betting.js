...
async function testCompleteBettingFlow() {
  ...
  // Use a completely new player ID that hasn't been used
  const playerId = 'P' + Math.floor(Math.random() * 100000);
  const boardNumber = 1; // Board 1 should be available
  const amount = 10;
  const stage = 'A';
  
  console.log(`🎯 Using new player ID: ${playerId}`);
  console.log(`🎯 Using board number: ${boardNumber}`);
